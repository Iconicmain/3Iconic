...
export async function GET(
  ...
  try {
    ...
    const expenseResponse = {
      ...
      station: expense.station || null,
      amount: Number(expense.amount) || 0,
      balance: expense.balance !== undefined && expense.balance !== null ? Number(expense.balance) : undefined,
      date: expense.date instanceof Date ? expense.date.toISOString().split('T')[0] : (expense.date || new Date().toISOString().split('T')[0]),
      status: expense.status || 'partially-paid',
      expenseType: expense.expenseType || 'recurrent',
      transactionCost: Number(expense.transactionCost) || 0,
