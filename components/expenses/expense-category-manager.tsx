...
export function ExpenseCategoryManager({ open, onOpenChange, onCategoryAdded }: ExpenseCategoryManagerProps) {
  ...
  const handleDeleteCategory = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/expense-categories/${deleteDialog.category._id}`, {
        method: 'DELETE',
      });
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to delete expense category');
      }
          ...
          <div className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                Existing Expense Categories ({categories.length})
              </h3>
              {fetching ? (
                ...
                <div className="text-center py-8 text-muted-foreground text-sm">
                  No expense categories found. Add your first category above.
                </div>
              ) : (
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="text-xs text-amber-600 dark:text-amber-400 mt-2 block">
                Note: This will not affect existing expenses with this category.
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
